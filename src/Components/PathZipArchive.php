...
class PathZipArchive extends \ZipArchive
{
    ...
    public function restore( $targetPath, $localName = null, $checksum = null )
    {
        ...
        if ( $checksum && $checksum != ( $_md5 = md5_file( $this->_zipFileName ) ) )
        {
            throw new \InvalidArgumentException(
                'The provided checksum does not match the file checksum of "' . $_md5 . '".'
            );
        }
        ...
        if ( true !== ( $_result = $this->open( $this->_zipFileName ) ) )
        {
            throw new FileSystemException(
                'Unable to open zip file "' . $this->_zipFileName . '". Error code: ' . $_result
            );
        }
     ...
     * @param string $path
     * @param string $localName
     * @param array  $excludedDirs Array of directories to exclude, relative to source path. "." and ".." are
